...
public class RecIntegral implements Serializable {
    ...
    // Однопоточный вариант можно оставить
    ...
    // Многопоточное вычисление через Runnable
    public double integrateSqrtMultiThreaded() throws InterruptedException {
        if (lower < 0 || upper < 0) {
            throw new IllegalArgumentException("Границы интегрирования не могут быть отрицательными.");
        }
        if (step <= 0) {
        ...
        for (int i = 0; i < THREAD_COUNT; i++) {
            double end = (i == THREAD_COUNT - 1) ? upper : start + partLength;
